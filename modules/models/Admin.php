...
class Admin extends ActiveRecord{
	...
	{
		return [
			['adminuser', 'required', 'message'=>'用户名不能为空', 'on'=>['login','seekpass','changepass','adminadd','changeemail']],
			...
			['adminemail', 'unique', 'message'=>'电子邮箱已经被注册', 'on'=>['adminadd','changeemail']],
			['adminuser', 'unique', 'message'=>'管理员已经存在', 'on'=>'adminadd'],
	...
	public function seekPass($data)
	{
		...
		if($this->load($data) && $this->validate()){
			...
			$mailer->setFrom('15600045558@163.com');
			$mailer->setTo($data['Admin']['adminemail']);
			$mailer->setSubject('慕课网-找回密码');
			if($mailer->send()){
				return true;
			}	
		}
